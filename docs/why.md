...
Most frameworks give you one agent with a list of tools. That's fine for a chatbot. But what if you want 50 personas running a simulation? Or a team of specialists that delegate to each other? Or a system that monitors, decides, and acts on its own?

InteractKit lets you build all of these the same way: **composable entities**.

An entity is a TypeScript class that does one thing. It remembers things (`@State`). It can do things (`@Tool`). You snap entities together and they become something bigger.
...
class Brain extends LLMEntity {
  @SystemPrompt()
  private systemPrompt = 'You are a helpful research assistant.';
...
### MCP Servers Are Just Entities

Any [MCP](https://modelcontextprotocol.io) server becomes an entity with one decorator:
...
Reference it from an LLMEntity and the LLM can use all of Slack's tools alongside your own. Your code, MCP servers, extension packages. They're all just entities with tools.
...
| `@MCP` | Any MCP server becomes an entity |
...
**One entity does one thing. They compose into agents. Agents compose into systems.**
